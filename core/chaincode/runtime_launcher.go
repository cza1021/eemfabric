...
func (r *RuntimeLauncher) Launch(ccid string, streamHandler extcc.StreamHandler) error {
	...
	isScalechaincode := strings.ContainsAny(ccid, string('+'))
	...
	if !alreadyStarted {
		...
		go func() {
			...
			if ccservinfo == nil {
				startFailCh <- errors.WithMessage(err, "chaincode is not install prove commit")
				return
			}
			if isScalechaincode {
				address := strings.Split(ccid, "+")[1]
				ccservinfo.Address = address
			}
			if err != nil {
