...
func (cs *ChaincodeSupport) Launch(ccid string) (*Handler, error) {
	if h := cs.HandlerRegistry.LbHandler(ccid); h != nil {
		return h, nil
	}
...
func (cs *ChaincodeSupport) LbHandleChaincodeStream(stream ccintf.ChaincodeStream, ccid string) error {
	handler := &Handler{
		Invoker:                cs,
		Keepalive:              cs.Keepalive,
		Registry:               cs.HandlerRegistry,
	...
	return handler.LbProcessStream(stream, ccid)
}
