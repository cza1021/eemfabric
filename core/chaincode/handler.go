...
func (h *Handler) LbProcessStream(stream ccintf.ChaincodeStream, ccid string) error {
	defer h.deregister()
	...
	for {
		...
		case rmsg := <-msgAvail:
			...
			case rmsg.msg == nil:
				...
				return err
			default:
				err := h.LbhandleMessage(rmsg.msg, ccid)
				if err != nil {
...
// handleMessage is called by ProcessStream to dispatch messages.
func (h *Handler) LbhandleMessage(msg *pb.ChaincodeMessage, ccid string) error {
	chaincodeLogger.Debugf("[%s] Fabric side handling ChaincodeMessage of type: %s in state %s", shorttxid(msg.Txid), msg.Type, h.state)
	...
	switch h.state {
	case Created:
		return h.LbhandleMessageCreatedState(msg, ccid)
	case Ready:
		return h.handleMessageReadyState(msg)
	default:
		return errors.Errorf("handle message: invalid state %s for transaction %s", h.state, msg.Txid)
...
func (h *Handler) LbhandleMessageCreatedState(msg *pb.ChaincodeMessage, ccid string) error {
	switch msg.Type {
	case pb.ChaincodeMessage_REGISTER:
		h.LbHandleRegister(msg, ccid)
	default:
		return fmt.Errorf("[%s] Fabric side handler cannot handle message (%s) while in created state", msg.Txid, msg.Type)
	}
	return nil
...
func (h *Handler) LbHandleRegister(msg *pb.ChaincodeMessage, ccid string) {
