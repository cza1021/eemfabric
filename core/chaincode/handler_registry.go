...
func selectRandomHandler(handlers map[string]*Handler) *Handler {
	keys := make([]string, 0, len(handlers))
	for k := range handlers {
		keys = append(keys, k)
	}

	rand.Seed(int64(rand.Intn(len(keys))))
	selectedKey := keys[rand.Intn(len(keys))]

	return handlers[selectedKey]
}

func getHandlerWithPrefix(m map[string]*Handler, prefix string) map[string]*Handler {
...
func (r *HandlerRegistry) LbHandler(ccid string) *Handler {

	handlers := getHandlerWithPrefix(r.handlers, ccid)
	if len(handlers) == 0 {
		return nil
	}
	return selectRandomHandler(handlers)
...
func (r *HandlerRegistry) Register(h *Handler) error {
	...
	if h == nil {
		return errors.Errorf("hander is null ")
	}
